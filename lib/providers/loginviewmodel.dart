...
{
  String erronMessage = "";// status là một biến số nguyên dùng để xác định trạng thái 
  int status = 0; // 0 - not login, 1 - waiting, 2 - error, 3 - already logged
  LoginRepository loginRepo = LoginRepository();
  Future <void> login(String username, String password) // async thực hiện các tác vụ mà không cần chờ kết quả của tác vụ trước đó hoàn thành 
  async {
    status = 1;
    notifyListeners();
    try{
